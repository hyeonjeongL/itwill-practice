...
public class AladinPage {
		...
		for (String url : urlList) {
			...
			try {
				...
				for (Element el : bookDiv) {
					...
					try {
						writer = info.split("\\|")[0];
						if(writer.length()>50) writer = writer.substring(0,50);
						publisher = info.split("\\|")[1];
						if(publisher.length()>100) publisher = publisher.substring(0,100);
					...
					BookDto bookData = 
							new BookDto(0,Integer.parseInt(gubun), title,writer, publisher,imgSrc,img, bookInfo, pubYear, itemId, pageCnt, null);
	...
	private int getItemId(String href) {
		String key = "";
