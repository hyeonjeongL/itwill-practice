...
public class UserController {
	...
	public String user_write_action_pose(@ModelAttribute User user, Model model) throws Exception {
		String forward_path = "";
		/*
		 * -1:아이디중복 1:회원가입성공
		 */
	...
	public String user_login_action_post(@ModelAttribute(value = "fuser") User user, HttpServletRequest request) throws Exception {
		...
		switch (result) {
		case 0:
			request.setAttribute("msg1", user.getUserId()+"는 존재하지 않는 아이디입니다.");
			forwardPath="user_login_form";
			break;
		case 1:
			request.setAttribute("msg2", "패스워드가 일치하지 않습니다.");
			forwardPath="user_login_form";
			break;
		case 2:
	...
	@LoginCheck
	@GetMapping(value = {"/user_modify_form", "/user_login_action","/user_write_action","/user_modify_aciton", "/user_remove_action"})
